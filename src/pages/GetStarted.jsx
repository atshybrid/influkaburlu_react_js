...
export default function GetStarted() {
  ...
  const register = async (e) => {
    ...
    try {
      ...
      const reg = await apiClient.request('/auth/register', {
        method: 'POST',
      ...
      if (reg?.token) {
        saveTokens({
          ...
          refreshExpiresAt: reg?.refreshExpiresAt,
          user: reg?.user,
        });
      } else {
        // If register doesn't return a token, immediately login with phone+MPIN.
        const auth = await apiClient.request('/auth/login-mobile', {
          method: 'POST',
          body: JSON.stringify({ phone: payload.phone, mpin }),
          skipAuth: true,
        });
        saveTokens({
          token: auth?.token,
      ...
      window.setTimeout(() => {
        ...
        const resolvedRole = normalizeRole(storedUser?.role) || getRoleFromToken(localStorage.getItem('auth.token')) || role;
        window.location.href = resolvedRole === 'influencer' ? '/dashboard-influencer' : '/dashboard-advertiser';
      }, 600);
    } catch (e) {
